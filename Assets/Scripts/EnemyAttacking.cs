...
public class EnemyAttacking : MonoBehaviour
{
    ...
    [SerializeField] private float attackInterval = 10.0f;
    [SerializeField] private float minimumAttackInterval = 5.0f;
    [SerializeField] private float waitBeforeThrowing = 2.0f;
    [SerializeField] private float waitAfterThrowing = 2.0f;
    ...
    public IEnumerator Attack()
    {
        ...
        while (true)
        {
            if (agent.velocity.magnitude < 0.01f) isMoving = false;
            else isMoving = true;
            ...
            if (isMoving == false)
            {
                ...
                if (timeSinceLastAttack > minimumAttackInterval)
                {
                    ...
                    if (randomNumber > (1f / p - 1) && isMoving == false)
                    {
                        ...
                        if (_coroutineMove != null) { StopCoroutine(_coroutineMove); Debug.Log("Stop MoveToPoints"); }
                        yield return StartCoroutine(ThrowStone());
                        if (_coroutineMove != null) { StartCoroutine(_coroutineMove); }
    ...
    private IEnumerator ThrowStone()
    {
        ...
        Vector3 throwPos = originPos + new Vector3(Random.Range(-5.0f, 5.0f), 0, Random.Range(-2.0f, 2.0f));
        bool isThrown = false;
        ...
        while (isThrown == false)
        {
            ...
            if (agent.velocity.magnitude < 0.01f && distance.magnitude < 1.0f)
            {
                ...
                if (Mathf.Abs(diffRot.y) < 0.01f)
                {
                    ...
                    yield return new WaitForSeconds(Random.Range(waitBeforeThrowing - 1.0f, waitBeforeThrowing + 1.0f));
                    ...
                    _stone.ThrowWithAngle(60f);
                    yield return new WaitForSeconds(Random.Range(waitAfterThrowing - 1.0f, waitAfterThrowing + 1.0f));
